...
# class OceanDataset(Dataset):
#     def __init__(self, directory, start_month_day='01-01', end_month_day='06-30', 
#                  sequence_length=5, prediction_horizon=1, predict_differences=True, transform=None):
#         self.directory = directory
#         self.start_month_day = start_month_day
...
#         data_files = []
#         start_month, start_day = map(int, self.start_month_day.split('-'))
#         end_month, end_day = map(int, self.end_month_day.split('-'))
...
#                 if (file_date.month > start_month or (file_date.month == start_month and file_date.day >= start_day)) and \
#                    (file_date.month < end_month or (file_date.month == end_month and file_date.day <= end_day)):
#                     data_files.append((file_date, os.path.join(self.directory, filename)))
        
#         # Сортируем по дате, чтобы данные были последовательны
#         data_files.sort(key=lambda x: x[0])
#         return data_files

#     def __len__(self):
#         return len(self.data_files) - self.sequence_length - self.prediction_horizon

#     def __getitem__(self, idx):
#         sequence = []
#         for i in range(self.sequence_length):
#             _, file_path = self.data_files[idx + i]
#             vector = np.load(file_path)
#             if self.transform:
#                 vector = self.transform(vector)  # Примените трансформацию, которая уже должна вернуть тензор
#             sequence.append(vector.clone().detach())  # Убедитесь, что тензор отключен от графа вычислений
        
#         last_sequence_vector = sequence[-1]
        
#         _, target_path = self.data_files[idx + self.sequence_length + self.prediction_horizon - 1]
#         target_vector = torch.tensor(np.load(target_path), dtype=torch.float32).clone().detach()

#         if self.predict_differences:
#             target = target_vector - last_sequence_vector
#         else:
#             target = target_vector
